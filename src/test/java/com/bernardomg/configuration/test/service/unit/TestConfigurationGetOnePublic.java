...
@DisplayName("Configuration service - get one public")
public class TestConfigurationGetOnePublic {
    ...
    void testGetOnePublic_Existing() {
        final Optional<Configuration> configuration;

        // GIVEN
        given(configurationRepository.findOnePublic(ConfigurationConstants.CODE))
            .willReturn(Optional.of(Configurations.stringValue()));

        // WHEN
        configuration = service.getOnePublic(ConfigurationConstants.CODE);
    ...
    void testGetOnePublic_NotExisting() {
        final ThrowingCallable execution;

        // GIVEN
        given(configurationRepository.findOnePublic(ConfigurationConstants.CODE)).willReturn(Optional.empty());

        // WHEN
        execution = () -> service.getOnePublic(ConfigurationConstants.CODE);
