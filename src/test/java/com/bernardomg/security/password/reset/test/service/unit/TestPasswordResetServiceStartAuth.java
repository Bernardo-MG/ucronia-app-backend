...
class TestPasswordResetServiceStartAuth {
    ...
    private final void loadCredentialsExpiredUser() {
        ...
        Mockito.when(user.getUsername())
            .thenReturn(USERNAME);
        Mockito.when(user.isAccountNonExpired())
            .thenReturn(true);
        Mockito.when(user.isAccountNonLocked())
            .thenReturn(true);
        Mockito.when(user.isCredentialsNonExpired())
            .thenReturn(false);
    ...
    private void loadPersistentUser() {
        ...
        Mockito.when(repository.findOneByEmail(EMAIL))
